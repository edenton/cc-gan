...
local function x2p(data, perplexity, tol)
  ...
  local eps = 1e-10
  local N = data:size(1)
  local D     = torch.Tensor(N, N)
  local y_buf = torch.Tensor(data:size())
  local row_D = torch.Tensor(N, 1)
